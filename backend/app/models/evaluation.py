...
class EvalSummary(Base):
    ...
    colleague_avg_score = Column(Numeric(6, 2), default=0,
                                 comment="同事/部门员工评分平均分（业务/产研=同事均值，基层管理/公共=部门员工均值）")
    colleague_count = Column(Integer, default=0, comment="同事/部门员工评价人数")
    superior_score = Column(Numeric(6, 2), default=0, comment="上级领导评分（多人取平均）")
    dept_leader_score = Column(Numeric(6, 2), default=0, comment="部门领导评分（多人取平均）")
    manager_mutual_score = Column(Numeric(6, 2), default=0, comment="基层管理互评平均分（仅基层管理人员）")
